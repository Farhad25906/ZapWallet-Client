...
const SendMoney = () => {
  ...
  const [formData, setFormData] = useState<SendMoneyRequest>({
    toWalletNumber: "+88",
    amount: "",
  });

  const [errors, setErrors] = useState<SendMoneyErrors>({});
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errors[name as keyof SendMoneyErrors]) {
      setErrors((prev) => ({
        ...prev,
        [name]: undefined,
      }));
  ...
  const validateForm = (): boolean => {
    const newErrors: SendMoneyErrors = {};
  ...
  const handleSubmit = async (): Promise<void> => {
    try {
      const payload: SendMoneyRequest = {
        toWalletNumber: formData.toWalletNumber,
        amount: formData.amount,
      };

      const response = await sendMoney(payload).unwrap();
      ...
      const transactionReceipt: TransactionReceipt = {
        ...
        fee: response?.data?.fee?.systemFee,
      };
                    ...
                    <Alert className="bg-[#ffd8af]/10 border-[#ffd8af]/30">
                      ...
                      <AlertDescription>
                        ...
                        <p className="text-slate-700">
                          ...
                          recipient's wallet immediately. This action is
                          irreversible.
                        </p>
                      </AlertDescription>
                    </Alert>
