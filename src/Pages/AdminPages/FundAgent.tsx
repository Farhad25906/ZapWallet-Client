...
import { useAgentAddMoneyMutation } from "@/redux/features/wallet/wallet.api";
...
// Types based on your existing type definitions
...
const FundAgent: React.FC = () => {
  const [addMoney, { isLoading }] = useAgentAddMoneyMutation();
  const { data } = useMyInfoQuery(undefined);

  const [formData, setFormData] = useState<FundAgentRequest>({
  ...
  const handleSubmit = async (): Promise<void> => {
    try {
      const payload: FundAgentRequest = {
        agentWalletNumber: formData.agentWalletNumber,
        amount: formData.amount,
      };

      const response = await addMoney(payload).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-3 sm:p-4 md:p-6">
                  ...
                  <div className="bg-white rounded-xl p-4 sm:p-6 border-2 border-[#009689]/20 space-y-3 sm:space-y-4">
                    <div className="flex items-center justify-between pb-3 sm:pb-4 border-b-2 border-[#ffd8af]/30">
                      ...
                      <div className="text-right">
                        ...
                        <Badge className="bg-[#009689] text-white hover:bg-[#009689] mt-1 text-xs">
                          SUPER_ADMIN
                        </Badge>
                      </div>
                    </div>
                    ...
                    <Alert className="bg-[#ffd8af]/10 border-[#ffd8af]/30">
                      ...
                      <AlertDescription className="text-xs sm:text-sm">
                        ...
                        <p className="text-slate-700">
                          Once confirmed, the amount will be added to the
      ...
      <TransactionSuccessModal
        ...
        title="Fund Agent Successful!"
        description="Money has been added to agent wallet successfully"
      />
    </div>
  );
};
