...
import TransactionSuccessModal, { type TransactionReceipt } from "@/components/modules/TransactionSuccessModal";
...
const CashIn = () => {
  ...
  const [formData, setFormData] = useState<CashInRequest>({
    toWalletNumber: "+88",
    amount: "",
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errors[name as keyof CashInErrors]) {
      setErrors((prev) => ({
        ...prev,
        [name]: undefined,
      }));
  ...
  // Zod-like validation function
  const validateForm = (): boolean => {
    const newErrors: CashInErrors = {};

    // Validate phone number
    const { pattern, message } = validationRules.toWalletNumber;
    if (!pattern.test(formData.toWalletNumber)) {
      newErrors.toWalletNumber = message;
    }
  ...
  const handleSubmit = async (): Promise<void> => {
    try {
      const payload: CashInRequest = {
        toWalletNumber: formData.toWalletNumber,
        amount: formData.amount,
      };

      const response = await cashIn(payload).unwrap();
      console.log(response.data.transaction._id);
      
      const transactionReceipt: TransactionReceipt = {
        transactionId: response.data.transaction._id,
        amount: parseFloat(formData.amount as string),
        fromNumber: data?.data?.phone || "N/A",
        toNumber: formData.toWalletNumber,
      ...
      toast.error(error?.data?.message || "Failed to fund agent");
      console.error(error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-3 sm:p-4 md:p-6">
              ...
              {showConfirmation ? (
                    ...
                    <Alert className="bg-[#ffd8af]/10 border-[#ffd8af]/30">
                      ...
                      <AlertDescription className="text-xs sm:text-sm">
                        ...
                        <p className="text-slate-700">
                          Once confirmed, the amount will be added instantly to the user's wallet. This action is irreversible, ensure same amount is collected first from user.
                        </p>
                      </AlertDescription>
                    </Alert>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center">
                  ...
                  <p className="text-xs sm:text-sm text-slate-500 px-4">
                    Enter user wallet number and amount to preview the
                    transaction
                  </p>
                </div>
              )}
      ...
      <TransactionSuccessModal
        ...
        description="Money has been added to user wallet successfully"
      />
    </div>
  );
};
